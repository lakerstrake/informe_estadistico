  ...
  return (
    <Section id="metodologia" kicker="De dónde salen los datos" title="Base de datos y metodología">
      <div className="grid md:grid-cols-2 gap-8 items-start">
        <Reveal>
          <Card className="p-6">
            <p className="font-bold text-azul-dark mb-4">Ficha técnica</p>
            <table className="w-full text-sm">
              <tbody>
                {ficha.map(([k, v]) => (
                  <tr key={k} className="border-b border-azul/5 last:border-0 hover:bg-azul/[0.03] transition-colors">
                    <td className="py-2.5 pr-3 font-semibold text-gris w-2/5 text-[0.8rem] uppercase tracking-wide">{k}</td>
        ...
        <Reveal delay={0.1}>
          <Card className="p-6">
            <p className="font-bold text-azul-dark mb-3">Fórmula de la razón de prevalencia</p>
